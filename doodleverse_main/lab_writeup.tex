...
\fbox{\begin{minipage}{40em}
\textbf{Consider this as the clustering problem $X = DW$. What do $X,D,W$ look like? Our search set contains 79 stars. If D is $n\times k$, what is $n$? What would be the effect of choosing smaller or larger values of $k$?}
\end{minipage}
}
...
\begin{enumerate}
  ...
  $$
  \hat{F} = \frac{F-mean(F)}{||F||_F} \quad\textrm{ and }\quad\hat{S} = \frac{S}{||S||_F} - mean(S)
  $$
